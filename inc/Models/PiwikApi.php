...
class PiwikApi {
	 ...
	 */
	function __construct( $siteId, $format = self::FORMAT_JSON, $period = self::PERIOD_DAY,
						 $date = self::DATE_YESTERDAY, $rangeStart = '', $rangeEnd = null ) {
		$env = include( OTB_DIR . '.env.php' );
		$this->_site = $env['piwik']['SITE_URL'];
		$this->_token = $env['piwik']['SITE_TOKEN'];
		$this->_siteId = $siteId;
		$this->_format = $format;
		$this->_period = $period;
	...
	public function getRange() {
		...
		} else { return $this->_rangeStart . ',' . $this->_rangeEnd;
		}
	}
	...
	private function _request( $method, $params = [], $optional = [] ) {
		...
		if (  0 === strcmp( $method, 'ImageGraph.get' ) ) {
			return $url;
		}
	 ...
	 * @param string $colors Use own colors instead of the default. The colors has to be in hexadecimal
	 *                      value without '#'
	 * @param array $optional
	 */
